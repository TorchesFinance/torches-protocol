...
module.exports = async function(deployer, network) {
    ...
    await deployer.deploy(TorchesConfig, "0x0000000000000000000000000000000000000000");
    ...
    console.log("Done to set torches config.", await  proxiedTorchestroller.torchesConfig());
    ...
    if (network == "kcctest" || network == "kcc") {
        ...
        console.log("Done to set price oracle.", await proxiedTorchestroller.oracle());
        addressFactory["PriceOracle"] = priceOracleAddress;
        await deployer.deploy(wrappedNativeDelegate);
        await deployer.deploy(wrappedNativeDelegator, kcsToken, Unitroller.address, InterestModel.address, 0.02e18.toString(), "Torches KCS", "tKCS", 18, admin, wrappedNativeDelegate.address, "0x0");
        ...
        await proxiedTorchestroller._setCollateralFactor(wrappedNativeInstance.address, collateralFactor);
        console.log("Done to set collateral factor %s for tKCS %s", collateralFactor, wrappedNativeInstance.address);
