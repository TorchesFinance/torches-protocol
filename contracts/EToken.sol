...
import "./Enzotroller.sol";
import "./IERC3156FlashBorrower.sol";
import "./ILiquidityGauge.sol";

contract EToken is CToken {
    ...
    function notifySavingsChange(address addr) internal {
        EnzoConfig enzoConfig = Enzotroller(address(comptroller)).enzoConfig();
        ILiquidityGauge liquidityGauge = enzoConfig.liquidityGauge();
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        (vars.liquidatorSeizeTokens, vars.safetyVaultTokens) = enzoConfig.calculateSeizeTokenAllocation(seizeTokens, liquidationIncentive);
        address safetyVault = enzoConfig.safetyVault();
        /*
    ...
    function maxFlashLoan(address token) external view returns (uint256) {
        ...
        return Enzotroller(address(comptroller)).getFlashLoanCap(address(this));
    }
    ...
    function getFlashFeeInternal(address token, uint256 amount) internal view returns (uint256) {
        token;
        return Enzotroller(address(comptroller)).enzoConfig().getFlashFee(msg.sender, address(this), amount);
    }
    ...
    function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 amount, bytes calldata data) external returns (bool) {
        ...
        Enzotroller(address(comptroller)).flashLoanAllowed(address(this), address(receiver), amount);
